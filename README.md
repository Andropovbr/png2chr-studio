...
- **Sprite Sheet & Animation Workflow:**
  ...
  - Deterministic per-scanline sprite pressure validation, warning at 6–8 intersecting sprites and blocking export readiness above the NES limit of 8, with frame-edge clipping respected.
  - Scene coordinate and visibility diagnostics preserve legitimate partial clipping, warn only when a complete metasprite instance is outside the 256×240 picture, and report unsigned 8-bit OAM coordinate wrapping as informational.
...
- **Deliver & Export Workspace:** Consolidated delivery hub showing project readiness status, per-asset CHR resource accounting, Nametable-to-CHR backing validation for Playfields, domain and ownership integrity diagnostics with direct links to relevant editing workspaces, and unified downloads for all production binary and source artifacts across Tileset, Playfield, and Animation modes.
...
For a focused Scene pipeline pass:
...
This covers project migration/save-reload, stable animation resolution, anchor/flips, multi-instance pointer and keyboard edits, ordering, palette/CHR context, cross-workspace navigation, and transient playback teardown/remount. Scene has no standalone export format or separate authoritative coverage threshold; its supported external boundary is `scenePreview` inside `.p2c`.
