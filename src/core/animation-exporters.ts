...
function estimatedRomBytes(model: AnimationProjectModel): number {
  ...
  return (
    Math.max(1, flat.sprites.length) * 6 +
...
; Sprite entry (5 bytes): signed X, signed Y, CHR tile word, NES OAM attributes.
...
`;
  const spriteRows = flat.sprites.length
    ? flat.sprites.map(
        (sprite) =>
          `    .byte ${asmHex(signedByte(sprite.x))}, ${asmHex(signedByte(sprite.y))}\n    .word ${asmWord(sprite.tile)}\n    .byte ${asmHex(sprite.attributes)}`,
      )
    : ['    .byte $00, $00\n    .word $0000\n    .byte $00 ; sentinel'];
