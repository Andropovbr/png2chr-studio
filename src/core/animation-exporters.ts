...
`;
  ...
  const animationRows = flat.animations.map(
    (animation) =>
      `    { ${String(animation.frameOffset)}, ${String(animation.frameCount)}, ${String(animation.widthTiles)}, ${String(animation.heightTiles)}, ${String(animation.category)} },`,
  );
...
; Animation entry (6 bytes): frame offset word, frame count, width tiles,
;                            height tiles, type (0 idle, 1 movement).
...
`;
  ...
  const animationRows = flat.animations.map(
    (animation) =>
      `    .word ${String(animation.frameOffset)}\n    .byte ${String(animation.frameCount)}, ${String(animation.widthTiles)}, ${String(animation.heightTiles)}, ${String(animation.category)}`,
  );
