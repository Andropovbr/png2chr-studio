...
- **Migração v1 para v2:** `deserializeProject` aceita v1, cria catálogo/IDs/contextos deterministicamente, move a semântica de Base CHR para `graphics.baseChr` e converte cada tela Playfield legada em Background Map. `serializeProject` grava somente v2 e não persiste `playfield`.
...
- **Background Maps:** São a única representação persistida de tela. Cada mapa armazena 960 células lógicas, 240 atribuições de paleta e, opcionalmente, `collision.cells` (dados de gameplay) e `procedural.features`. Colisão é exportada pelo serializador `.col` existente, mas não participa da validade gráfica NES de `.nam`, `.atr` ou `.chr`.
...
- **Compilação e Mapeamento de CHR (Runtime-Derived):** `compileProjectGraphics` produz a CHR-ROM estática final de 8 KiB, o manifesto físico imutável, placements lógicos, índices locais de Nametable/OAM, usos e fatos de capacidade. Esse resultado é a autoridade de alocação do projeto, mas **não é serializado no `.p2c`**; v2 persiste intenção lógica e políticas, nunca placement físico compilado. Delivery usa o resultado para readiness, fatos dependentes de placement e bloqueio de artefatos gráficos; ausência, fonte não suportada e falha de compilação são exibidas explicitamente, enquanto fatos de hardware/runtime sem compilação permanecem desconhecidos. `.col` é independente da validade da compilação gráfica.
