...
- **Compilação e Mapeamento de CHR (Runtime-Derived):** `compileProjectGraphics` produz a CHR-ROM estática final de 8 KiB, o manifesto físico imutável, placements lógicos, índices locais de Nametable/OAM, usos e fatos de capacidade. Esse resultado é a autoridade de alocação do projeto, mas **não é serializado no `.p2c`**; v2 persiste intenção lógica e políticas, nunca placement físico compilado. As projeções antigas de CHR Memory, diagnósticos e Delivery ainda aguardam migração e não alteram esse contrato.
