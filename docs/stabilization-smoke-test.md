...
| 9    | In **Deliver & Export**, inspect animation frames with 33 and 64 sprites, then one with 65 sprites.                                           | Frames above 32 show a sprite-pressure warning; 64 remains warning-only; frames above 64 show an OAM capacity error.                                                                     |
| 10   | Enable an exported mirrored movement direction and reopen **Deliver & Export**.                                                               | OAM and scanline diagnostics appear once for the canonical editable animation; the generated mirror does not duplicate them.                                                             |
| 11   | Save project as `.p2c`, reload the application, and open the saved file.                                                                      | Project state reloads completely; exports re-generated from reloaded model are 100% bit-for-bit identical to initial exports.                                                            |

## Graphics Assets & Background Maps smoke test
...
| 13   | Compose one 16×16 region from source Tiles that use two different palette contexts, with Base CHR or a Reservation before their allocated slots. | Deliver & Export shows one informational Attribute Table diagnostic based on the compiled physical assignments, and its action opens Background. No diagnostic is emitted when source palette provenance is unavailable.        |

## NES Validation final quality pass (Milestone 11)

1. Open Deliver & Export in Tileset, Playfield and Animation without loading the
   mode's primary content. Each mode must show **Action Required**, not “Ready”.
2. Verify an info-only project, a warning-only project and an error project.
   Status must be respectively **Ready**, **Ready with Warnings** and
   **Action Required**; the summary must count info, warnings and blockers.
3. Use each diagnostic action. OAM/scanline/visibility opens Animation,
   Nametable backing opens CHR Memory, and Attribute Table opens Background.
4. Switch locale between English and pt-BR. Diagnostic text, download actions,
   navigation labels and `Base CHR` terminology must remain localized and
   consistent.
