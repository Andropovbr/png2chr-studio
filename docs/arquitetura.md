...
- **Validação de capacidade OAM por frame:** `src/core/oam-diagnostics.ts` conta cada sprite gerado como uma entrada OAM. Mais de 32 entradas gera aviso de pressão de sprite (não é limite de hardware); mais de 64 gera erro, pois excede as 64 entradas da OAM do NES. Variantes de direção geradas por espelhamento horizontal não repetem o fato da animação canônica editável.
- **Validação de pressão por scanline:** `src/core/nes-sprite-diagnostics.ts` projeta as coordenadas relativas do metasprite de volta ao espaço local do frame e conta a interseção vertical de cada sprite 8×8 em cada linha de pixels. O intervalo é semiaberto (`y <= scanline < y + 8`) e recortado por `0..frameHeight - 1`: sprites totalmente fora do frame não contam e sprites parcialmente visíveis contam apenas nas linhas intersectadas. Entre 6 e 8 sprites concorrentes gera aviso; acima de 8 gera erro de limite de hardware. O **Deliver & Export** apresenta deterministicamente a scanline de maior pressão por frame, preservando o primeiro índice em caso de empate; variantes horizontais derivadas não duplicam a mesma geometria vertical.
- **Validação de coordenadas e visibilidade da Scene:** `analyzeSceneInstanceVisibility(...)`, no mesmo módulo, resolve somente IDs canônicos não ambíguos e analisa cada frame real de instâncias persistidas visíveis. A projeção combina âncora, origem e offsets dos sprites 8×8 efetivamente presentes no metasprite. Clipping parcial nas bordas `256×240` não gera diagnóstico; uma instância cujos sprites estejam todos fora da imagem gera `warning` com os limites da composição; coordenadas absolutas fora de `0..255`, antes da interpretação do byte OAM, são consolidadas em um único `info` por instância/frame com os valores após wrap. Variantes espelhadas derivadas não duplicam fatos, referências não resolvidas continuam sob os diagnósticos próprios, e o **Deliver & Export** consome diretamente esses fatos puros.
...
- **Validação de Consistência entre Nametable e CHR (`src/core/nes-background-diagnostics.ts`):**
  ...
  - `analyzeAttributeTableAssignments` verifica, por região canônica 16×16 px, se Tiles com proveniência conhecida exigem contextos de paleta distintos. A identidade física vem exclusivamente de `BackgroundProjectModel.resolvedCells`; a proveniência de paleta permanece ligada a `LogicalTileKey`, sem inferência por igualdade com `sourceTileIndex`. Emite apenas fatos `info`, pois a recoloração pode ser intencional; não repete validação de tamanho/faixa nem reempacota a Attribute Table. Contexto ausente permanece indeterminável.
